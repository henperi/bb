          ...
          if (admin_mobile) {
            ...
            Admin.findOne({}, (err, result) => {
              if (err) return next(err);
                ...
                req.flash(
                  "errorMsg",
                  "Unable to register, please contact an admin"
                );
                res.redirect("back");
              } else {
                ...
                newAdmin.save((err, registeredAdmin) => {
                  if (err) throw err;
                    ...
                    console.log(
                      `Admin saved to the database ${registeredAdmin}`
                    );
    ...
    async.parallel(
      {
        ...
        indirectCommission: callback => {
          AgentCommissionHistory.aggregate([
            {
              $match: {
                "commissionsData.com_received_by_admin.received_by":
                  req.user._id
              }
            },
            {
        ...
        agentCommission: callback => {
          AgentCommissionHistory.aggregate([
            {
              $match: {
                "commissionsData.com_received_by_agent.received_by":
                  req.user._id
              }
            },
            {
      ...
      (err, results) => {
        if (err) return next(err);
          ...
          if (results.directCommission[0]) {
            console.log(
              "directCommission: ",
              results.directCommission[0].total
            );
          ...
          if (results.indirectCommission[0]) {
            console.log(
              "indirectCommission: ",
              results.indirectCommission[0].total
            );
                ...
                (err, foundAdmin) => {
                  if (err) return next(err);
                    ...
                    if (!foundAdmin) {
                      req.flash(
                        "warningMsg",
                        "This Admin email was not found Try again"
                      );
                        ...
                        (err, result) => {
                          if (err) return next(err);
                            ...
                            if (!result) {
                              req.flash(
                                "warningMsg",
                                "This Admin wallet was not found Try again"
                              );
                              res.redirect(
                                `/admins/fund/search/${req.params.role}`
                              );
                            } else {
                ...
                (err, result) => {
                  if (err) return next(err);
                    ...
                    if (!result[0]) {
                      req.flash(
                        "warningMsg",
                        "No user with this wallet id or mobile was found, try again"
                      );
                      res.redirect(`/admins/fund/search/${req.params.role}`);
                    } else {
                      // console.log('adminWallet', adminWallet)
    ...
    AdminWallet.findOne(
      ...
      (err, adminWallet) => {
        if (err) return next(err);
          ...
          SendFundsHistory.find({ sender_wallet_id: req.user.mobile })
            .sort({ _id: -1 })
            .exec((err, sentFunds) => {
              if (err) return next(err);
              else {
    ...
    async.parallel(
      {
        ...
        receivedFunds: callback => {
          SendFundsHistory.find({ receiver_wallet_id: req.user.mobile })
            .sort({ _id: -1 })
            .exec(callback);
        }
      },
...
router.get(
  "/commissions/indirect-commissions",
  ensureAuth,
  (req, res, next) => {
    if (req.user.role == "Super-Admin") {
      AdminWallet.findOne(
...
//for agents
router.get(
  "/commissions/received-commissions",
  ensureAuth,
  (req, res, next) => {
...
router.post(
  "/commissions/update-agent-commission",
  ensureAuth,
  (req, res, next) => {
    req.checkBody("percentChargeOnAdmins", "Percentage is required").notEmpty();
    req
      .checkBody("cappedChaargeOnAdmins", "Capped Amount is required")
      .notEmpty();
        ...
        CommissionSettings.update(
          ...
          err => {
            if (err) return next(err);
              ...
              req.flash(
                "successMsg",
                "Agent Commissions Settings Updated Successfully"
              );
...
router.post(
  "/commissions/update-user-commission",
  ensureAuth,
  (req, res, next) => {
        ...
        CommissionSettings.update(
          ...
          err => {
            if (err) return next(err);
              ...
              req.flash(
                "successMsg",
                "User Commissions Settings Updated Successfully"
              );
...
function createAdmin(req, res, next) {
  console.log(
    "loging userData" + req.body.firstname,
    req.body.lastname,
    req.body.mobile,
    req.body.email,
    req.body.password
  ...
  if (errors) {
    ...
    if (account_type == "User") {
      if (req.user.role != "Super-Admin" && req.user.role != "Manager") {
        req.flash(
          "errorMsg",
          `You don't have priviledges to create a ${account_type} account`
        );
        res.redirect("back");
      } else {
              ...
              if (admin_mobile) {
                ...
                newUser.save((err, registeredAdmin) => {
                  if (err) {
                    ...
                    console.log(
                      `User saved to the database ${registeredAdmin.firstname}`
                    );
                    ...
                    newUserWallet.save((err, registerdWallet) => {
                      if (err) return next(err);
                        ...
                        console.log(
                          `UserWallet saved to the database ${
                            registerdWallet.wallet_id
                          }`
                        );
                      }
                    });
                    ...
                    req.flash(
                      "successMsg",
                      `You have registered a new ${account_type} successfully, password is ${
                        registeredAdmin.p_check
                      }`
                    );
                    res.redirect("back");
                  }
        ...
        Admin.findOne(emailQuery, (err, admin_email) => {
          ...
          if (admin_email) {
            ...
            Admin.findOne(mobileQuery, (err, admin_mobile) => {
              ...
              if (admin_mobile) {
                console.log(
                  `This admin mobile exists already: ${admin_mobile}`
                );
                req.flash("errorMsg", "This agent mobile has been taken");
                res.redirect("back");
              } else {
                ...
                newAdmin.save((err, registeredAdmin) => {
                  if (err) {
                    ...
                    console.log(
                      `Agent saved to the database ${registeredAdmin.firstname}`
                    );
                    ...
                    newAdminWallet.save((err, registerdWallet) => {
                      if (err) return next(err);
                        ...
                        console.log(
                          `AdminWallet saved to the database ${
                            registerdWallet.wallet_id
                          }`
                        );
                      }
...
function genFunds(req, res, next) {
  ...
  if (req.user.role != "Super-Admin") {
    ...
    if (isNaN(amount)) {
      req.flash(
        "warningMsg",
        `Please enter a valid amount: "${amount}" is not a number `
      );
      res.redirect("back");
    } else {
      if (amount <= 0) {
        req.flash(
          "infoMsg",
          `You can't generate anything bellow ${min_amount} `
        );
        res.redirect("back");
      } else {
        ...
        AdminWallet.findOne(mainAdminQuery, (err, adminWallet) => {
          if (err) return next(err);
            ...
            req.flash(
              "warningMsg",
              `Issue with your account, you can't generate right now`
            );
            res.redirect("back");
          } else {
            AdminWallet.update(
                ...
                if (err) return next(err);
                  ...
                  newGenFundsHistory.save((err, History) => {
                    if (err) throw err;
                      ...
                      console.log(
                        `GenerateFundHistory saved to the database ${History}`
                      );

                      req.flash(
                        "successMsg",
                        `Accounted topped up by ${amount}, previous ballance: ${
                          adminWallet.ballance
                        } `
                      );
                      res.redirect("back");
                    }
...
function fundAnyAccount(req, res, next) {
  console.log(
    "loging Fund data: " + req.body.fund_amount,
    req.body.remark,
  ...
  if (
    ...
    req.flash(
      "errorMsg",
      `You do not have privileges to carry out this funding action`
    );
    res.redirect("back");
  } else {
    ...
    let adminCommission =
      req.body.com_to_admin || (adminPercentage / 100) * fund_amount;
    ...
    if (isNaN(fund_amount)) {
      req.flash(
        "warningMsg",
        `Please enter a valid amount: "${fund_amount}" is not a number `
      );
      res.redirect("back");
    } else {
      if (fund_amount <= min_amount) {
        req.flash(
          "warningMsg",
          `You can't fund anything bellow ${min_amount} `
        );
        res.redirect("back");
      } else {
        CommissionSettings.findOne({}, (err, commission) => {
          ...
          if (req.user.role == "Super-Admin" || req.user.role == "Manager") {
            if (wallet_type == "User") {
              ...
              adminCommission =
                adminCommission > capOnUser ? capOnUser : adminCommission;
                ...
                (err, adminWallet) => {
                  ...
                  if (!adminWallet) {
                    ...
                    if (fund_amount > adminWallet.ballance) {
                      req.flash(
                        "warningMsg",
                        `Your ballance is too low, cannot move N${fund_amount}, when you only have N${
                          adminWallet.ballance
                        ...
                        (err, result) => {
                          ...
                          if (err) return next(err);
                            ...
                            if (!result.userWallet) {
                              req.flash(
                                "warningMsg",
                                `Issue with this account, the account might be invalid`
                              );
                              res.redirect("back");
                            } else if (!result.foundUser) {
                              req.flash(
                                "warningMsg",
                                `Issue with this account, the user details was not found`
                              );
                              res.redirect("back");
                            } else {
                              const foundUser_fullname =
                                result.foundUser.firstname +
                                " " +
                                result.foundUser.lastname;
                              console.log(
                                `Found user:${
                                  result.foundUser.firstname
                                } with wallet id ${result.userWallet.wallet_id}`
                              );
                              // return null;
                              const new_fund_amount =
                                fund_amount - adminCommission; //deduct the commission from the funding amount

                              console.log(
                                `Attempting to deduct ${fund_amount} from ${
                                  req.user.role
                                }:${req.user.mobile}...`
                              );
                                ...
                                err => {
                                  if (err) return next(err);
                                    ...
                                    newFundsHistory
                                      ...
                                      .then(save => {
                                        console.log(
                                          `Fund:${fund_amount} moved from ${
                                            req.user.mobile
                                          } to ${wallet_id}...`
                                        );
                                        req.flash(
                                          "successMsg",
                                          `Account ${wallet_type}:${wallet_id} topped up by ${fund_amount}. commission of N${adminCommission} was charged on ${wallet_type}:${wallet_id} and added back to ${
                                            req.user.role
                                          }:${req.user.mobile} `
                                        );
                                        res.redirect("back");
              ...
              adminCommission =
                adminCommission > capOnAgent ? capOnAgent : adminCommission;
                ...
                AdminWallet.findOne(
                  ...
                  (err, superWallet) => {
                    ...
                    if (!superWallet) {
                      ...
                      req.flash(
                        "warningMsg",
                        `Your admin account was not found`
                      );
                      res.redirect("back");
                    } else {
                      ...
                      if (fund_amount > superWallet.ballance) {
                        ...
                        async.parallel(
                          {
                            adminWallet: callback => {
                              AdminWallet.findOne(adminWalletQuery).exec(
                                callback
                              );
                            },

                            foundAgent: callback => {
                              Admin.findOne({ mobile: wallet_id }).exec(
                                callback
                              );
                            }
                          },
                          (err, result) => {
                            if (err) return next(err);
                              ...
                              if (!result.adminWallet) {
                                ...
                                req.flash(
                                  "warningMsg",
                                  `Issue with this account, the Agent details was not found`
                                );
                                res.redirect("back");
                              } else {
                                const foundAgent_fullname =
                                  result.foundAgent.firstname +
                                  " " +
                                  result.foundAgent.lastname;

                                console.log(
                                  `Found Agent:(${foundAgent_fullname}) with wallet id ${
                                    result.adminWallet.wallet_id
                                  } `
                                );
                                // return null;
                                const new_fund_amount =
                                    ...
                                    if (err) return next(err);
                                      ...
                                      newFundsHistory
                                        ...
                                        .then(save => {
                                          ...
                                          req.flash(
                                            "successMsg",
                                            `${wallet_type} Account ${foundAgent_fullname}:${wallet_id} topped up by ${fund_amount}. commission of N${adminCommission} was charged on ${wallet_type}:${wallet_id} and added back to ${
                                              req.user.role
                                            }:${req.user.mobile} `
                                          );
                                          res.redirect("back");
              ...
              if (req.user.role != "Super-Admin") {
                ...
                req.flash(
                  "warningMsg",
                  `You cannot fund a fellow Managert: "${
                    req.body.com_to_admin
                  }" is not a number `
                );
                res.redirect("back");
                          ...
                          (err, result) => {
                            if (err) return next(err);
                              ...
                              if (!result.adminWallet) {
                                req.flash(
                                  "warningMsg",
                                  `Issue with this account, the Account wallet details might be invalid`
                                );
                                res.redirect("back");
                              } else if (!result.foundManager) {
                                req.flash(
                                  "warningMsg",
                                  `Issue with this account, the Manager details was not found`
                                );
                                res.redirect("back");
                              } else {
                                console.log(
                                  `Found user with wallet id ${
                                    result.adminWallet.wallet_id
                                  } `
                                );
                                // return null;
                                ...
                                const foundManager_fullname =
                                  result.foundManager.firstname +
                                  " " +
                                  result.foundManager.lastname;
                                    ...
                                    if (err) return next(err);
                                      ...
                                      newFundsHistory
                                        ...
                                        .then(save => {
                                          ...
                                          req.flash(
                                            "successMsg",
                                            `${wallet_type} Account ${foundManager_fullname}:${wallet_id} topped up by ${fund_amount}. commission of N${adminCommission} was charged on ${wallet_type}:${wallet_id} and added back to ${
                                              req.user.role
                                            }:${req.user.mobile} `
                                          );
                                          res.redirect("back");
                          ...
                          if (err) return next(err);
                            ...
                            if (!result.userWallet) {
                              ...
                              console.log(
                                `${
                                  result.foundUser.role
                                } Found: ${foundUser_fullname} with wallet id ${
                                  result.userWallet.wallet_id
                                } `
                              );

                              const new_fund_amount =
                                fund_amount - adminCommission; //deduct the commission from the funding amount
                              const agent_fund_amount =
                                fund_amount - comToAgent; //deduct the agent commission from the funding amount
                                    ...
                                    AdminWallet.update(
                                      {
                                        ...
                                        wallet_id:
                                          result.foundMainWallet.wallet_id
                                      },
                                  ...
                                  if (err) return next(err);
                                    ...
                                    let newAgentCommissionHistory = new AgentCommissionHistory(
                                      {
                                        commissionsData: {
                                          com_received_by_admin: {
                                            received_by:
                                              result.foundMainWallet.user_id,
                                    ...
                                    newFundsHistory
                                      ...
                                      .then(save => {
                                        ...
                                        req.flash(
                                          "successMsg",
                                          `${wallet_type} Account ${foundUser_fullname}:${wallet_id} topped up by 
                                                                    ${fund_amount}. commission of N${adminCommission} was charged on ${wallet_type}:${wallet_id}, 
                                                                    out of which ${comToAgent} was added back to ${
                                            req.user.role
                                          }:${req.user.mobile}, and 
